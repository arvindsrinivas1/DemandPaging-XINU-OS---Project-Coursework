...
/* emulate 386 "pushal" instruction */
	...
	*pushsp = pptr->pesp = (unsigned long)saddr;
	...
	//paging
	//setup PD for the new process.
	pd_frame = get_frm(&free_frame);

	//setup frame(pd_frame)
	frm_tab[pd_frame].fr_status = FRM_MAPPED;
	frm_tab[pd_frame].fr_pid = pid;
	frm_tab[pd_frame].fr_vpno = -1;
	frm_tab[pd_frame].fr_refcnt = 1;
	frm_tab[pd_frame].fr_type = FR_DIR;
	frm_tab[pd_frame].fr_dirty = 0;

	//assign pdbr and proctab fields
	...
	//setup pdes inside the PD for the new process
	pde = (pd_t*)(pptr->pdbr);

//	(OR)

//	pde =  pde = (pd_t*)((FRAME0 + pd_frame) * NBPG);
